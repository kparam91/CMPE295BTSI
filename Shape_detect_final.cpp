...
int Image_Segment_Function(int x_coord1, int y_coord1)
{
        cout<< "hello"<<endl;
	int R1= 0, B1= 0, G1 =0;
	...
	else if(((R1 >=R_min_R) && (R1 <= R_max_R)) && ((G1 >= G_min_R) && (G1 <= G_max_R)) && ((B1 >= B_min_R) && (B1 <= B_max_R)))//RED
	{
		R_low1 = R_min_R, R_high1 = R_max_R, G_low1 = G_min_R, G_high1 = G_max_R, B_low1 = B_min_R, B_high1 = B_max_R;
		cout <<"RED"<<endl;
		flag = 1;
	}
	else if(((R1 >=R_min_W) && (R1 <= R_max_W)) && ((G1 >= G_min_W) && (G1 <= G_max_W)) && ((B1 >= B_min_W) && (B1 <= B_max_W)))
	{
		R_low1 = R_min_W, R_high1 = R_max_W, G_low1 = G_min_W, G_high1 = G_max_W, B_low1 = B_min_W, B_high1 = B_max_W;
		cout <<"WHITE"<<endl;
		flag = 1;
	}
	else if(((R1 >=R_min_Bl) && (R1 <= R_max_Bl)) && ((G1 >= G_min_Bl) && (G1 <= G_max_Bl)) && ((B1 >= B_min_Bl) && (B1 <= B_max_Bl)))
	{
		...
		for(int i=0; i<result.size(); ++i)
    		{
			 if(result[i].size() == 4)
				...
				line(image_shape2,Point(x_coord1 + 5,y_coord1),Point(x_coord1-5,y_coord1), Scalar(0,255,0), 1, 8, 0  );
					...
					imshow("My Window", image_shape2);
					flag = Image_Segment_Function(x_coord1, y_coord1);	
					if(flag)
					{
						rectcount++;
						cout <<"Rectangle "<<" ";
			...
			else if(result[i].size() == 8)
			{
				...
				else if(((abs(result[i][3].x -result[i][4].x) >=0)
					&& (abs(result[i][3].x -result[i][4].x) <=25))
